...
assert(Array.isArray(CARD_POOL) && CARD_POOL.length >= 86, `CARD_POOL has at least 86 cards in current baseline (actual ${CARD_POOL.length})`);
...
for (const [id, spec] of Object.entries(expectedBaseline)) {
...
for (const id of [...CONTENT_BASELINE_IDS, ...SILENCE_BASELINE_IDS]) {
  const card = getCardById(id);
  if (!card) continue;
...
assert([...p0NewCoreCodes].sort().join(",") === "aoeEnemy1,buffAdjacent1,nextSpellMinus1,summonTwo1_1",
  `content baseline has exactly four added core effect codes (${[...p0NewCoreCodes].sort().join(",")})`);
assert(CONTENT_BASELINE_IDS.concat(SILENCE_BASELINE_IDS).every((id) => FACTIONS[getCardById(id).faction]), "baseline faction ids all resolve");
assert(CONTENT_BASELINE_IDS.concat(SILENCE_BASELINE_IDS).every((id) => ["aggro", "control", "neutral"].includes(getCardById(id).axis)), "baseline axis ids all resolve");
