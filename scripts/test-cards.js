...
assert(Array.isArray(CARD_POOL) && CARD_POOL.length >= 86, `CARD_POOL has at least 86 cards in R56 (actual ${CARD_POOL.length})`);
...
assert(KEYWORDS.silence?.label === "靜默" && /移除/.test(KEYWORDS.silence.desc), "R56 silence 關鍵字已註冊");
assert(TIDE_CHANCE === 0.03, "R56 tideforged 機率為 3%");
...
console.log("== R56 content baseline ==");
const r56Cards = R56_BASELINE_IDS.map((id) => getCardById(id));
const r56SilenceCards = R56_SILENCE_IDS.map((id) => getCardById(id));
assert(r56Cards.every(Boolean) && r56SilenceCards.every(Boolean), "R56 baseline 10 cards plus 2 silence cards exist");
assert(r56Cards.every((c) => c.image === null) && r56SilenceCards.every((c) => c.image === null), "R56 baseline images are null placeholders");
assert(r56Cards.every((c) => typeof c.flavor === "string" && c.flavor.trim().length >= 8)
  && r56SilenceCards.every((c) => typeof c.flavor === "string" && c.flavor.trim().length >= 8), "R56 flavor text is present");
const expectedR56 = {
...
for (const [id, spec] of Object.entries(expectedR56)) {
...
for (const id of [...R56_BASELINE_IDS, ...R56_SILENCE_IDS]) {
  const card = getCardById(id);
  if (!card) continue;
...
assert([...p0NewCoreCodes].sort().join(",") === "aoeEnemy1,buffAdjacent1,nextSpellMinus1,summonTwo1_1",
  `R56 baseline has exactly four added core effect codes (${[...p0NewCoreCodes].sort().join(",")})`);
assert(R56_BASELINE_IDS.concat(R56_SILENCE_IDS).every((id) => FACTIONS[getCardById(id).faction]), "R56 faction ids all resolve");
assert(R56_BASELINE_IDS.concat(R56_SILENCE_IDS).every((id) => ["aggro", "control", "neutral"].includes(getCardById(id).axis)), "R56 axis ids all resolve");
