...
assert(Array.isArray(CARD_POOL) && CARD_POOL.length >= 86, `CARD_POOL has at least 86 cards in R55 (actual ${CARD_POOL.length})`);
...
assert(KEYWORDS.silence?.label === "靜默" && /移除/.test(KEYWORDS.silence.desc), "R55 silence 關鍵字已註冊");
assert(TIDE_CHANCE === 0.03, "R55 tideforged 機率為 3%");
...
console.log("== R55 content baseline ==");
const r55Cards = R55_BASELINE_IDS.map((id) => getCardById(id));
const r55SilenceCards = R55_SILENCE_IDS.map((id) => getCardById(id));
assert(r55Cards.every(Boolean) && r55SilenceCards.every(Boolean), "R55 baseline 10 cards plus 2 silence cards exist");
assert(r55Cards.every((c) => c.image === null) && r55SilenceCards.every((c) => c.image === null), "R55 baseline images are null placeholders");
assert(r55Cards.every((c) => typeof c.flavor === "string" && c.flavor.trim().length >= 8)
  && r55SilenceCards.every((c) => typeof c.flavor === "string" && c.flavor.trim().length >= 8), "R55 flavor text is present");
const expectedR55 = {
...
for (const [id, spec] of Object.entries(expectedR55)) {
...
for (const id of [...R55_BASELINE_IDS, ...R55_SILENCE_IDS]) {
  const card = getCardById(id);
  if (!card) continue;
...
assert([...p0NewCoreCodes].sort().join(",") === "aoeEnemy1,buffAdjacent1,nextSpellMinus1,summonTwo1_1",
  `R55 baseline has exactly four added core effect codes (${[...p0NewCoreCodes].sort().join(",")})`);
assert(R55_BASELINE_IDS.concat(R55_SILENCE_IDS).every((id) => FACTIONS[getCardById(id).faction]), "R55 faction ids all resolve");
assert(R55_BASELINE_IDS.concat(R55_SILENCE_IDS).every((id) => ["aggro", "control", "neutral"].includes(getCardById(id).axis)), "R55 axis ids all resolve");
