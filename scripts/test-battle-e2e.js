...
async function run() {
  ...
  for (const vp of [{ w: 1280, h: 900, name: "桌面 1280x900" }, { w: 1366, h: 700, name: "矮桌機 1366x700" }, { w: 390, h: 844, name: "手機 390x844" }]) {
    ...
    if (vp.w === 1280) {
      ...
      const pwaCheck = await page.evaluate(async () => {
        ...
        const guard = {
          ...
          versionedRefs: /cards\.js\?v=card-battle-r47-v1/.test(battleHtml)
            && /battle\.js\?v=card-battle-r47-v1/.test(battleHtml)
            && /pack\.js\?v=card-battle-r47-v1/.test(packHtml)
            && /manifest\.webmanifest\?v=card-battle-r47-v1/.test(shellText),
        };
        return {
      ...
      assert(pwaCheck.manifestHref === "../manifest.webmanifest?v=card-battle-r47-v1"
      ...
      assert(/CACHE_VERSION/.test(pwaCheck.swText)
        ...
        && pwaCheck.swText.includes("card-battle-r47-v1")
        ...
        && pwaCheck.version === "card-battle-r47-v1"
        && pwaCheck.versionLabel.includes("card-battle-r47-v1")
        && pwaCheck.checked.version === "card-battle-r47-v1",
      ...
      assert(pwaCheck.guard.windowMs === 15000 && pwaCheck.guard.key === "card_sw_auto_reload_r47_v1"
    ...
    assert(battleSwGuard.key === "card_sw_auto_reload_r47_v1" && battleSwGuard.windowMs === 15000 && battleSwGuard.late === false,
    ...
    assert(packR36.textState.attr === "large" && packR36.textState.select === "large"
      && packR36.large > packR36.small && packR36.pwaVersion.includes("card-battle-r47-v1"),
      "開包戰績區顯示版本並可調整文字大小");
    assert(packR36.missionOpen && packR36.missionAria === "false" && /mission/.test(packMissionFocus),
      "開包任務抽屜開啟後焦點進入抽屜控制");
    assert(packR36.swGuard.key === "card_sw_auto_reload_r47_v1" && packR36.swGuard.windowMs === 15000 && packR36.swGuard.late === false,
