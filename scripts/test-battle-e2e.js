...
const MIME = { ".html": "text/html", ".js": "application/javascript", ".json": "application/json", ".webmanifest": "application/manifest+json", ".png": "image/png", ".webp": "image/webp", ".css": "text/css" };
...
async function run() {
  ...
  for (const vp of [{ w: 1280, h: 900, name: "桌面 1280x900" }, { w: 1366, h: 700, name: "矮桌機 1366x700" }, { w: 390, h: 844, name: "手機 390x844" }]) {
    ...
    if (vp.w === 1280) {
      ...
      const pwaCheck = await page.evaluate(async () => {
        ...
        const guard = {
          ...
          versionedRefs: /cards\.js\?v=card-battle-r71-v1/.test(battleHtml)
            && /battle\.js\?v=card-battle-r71-v1/.test(battleHtml)
            && /pack\.js\?v=card-battle-r71-v1/.test(packHtml)
            && /manifest\.webmanifest\?v=card-battle-r71-v1/.test(shellText),
        };
        return {
      ...
      assert(pwaCheck.manifestHref === "../manifest.webmanifest?v=card-battle-r71-v1"
      ...
      assert(/CACHE_VERSION/.test(pwaCheck.swText)
        ...
        && pwaCheck.swText.includes("card-battle-r71-v1")
        ...
        && pwaCheck.version === "card-battle-r71-v1"
        && pwaCheck.versionLabel.includes("card-battle-r71-v1")
        && pwaCheck.checked.version === "card-battle-r71-v1",
      ...
      assert(pwaCheck.guard.windowMs === 15000 && pwaCheck.guard.key === "card_sw_auto_reload_r71_v1"
    ...
    assert(battleSwGuard.key === "card_sw_auto_reload_r71_v1" && battleSwGuard.windowMs === 15000 && battleSwGuard.late === false,
      "對戰頁 SW 自動更新守衛超過 15 秒不會自動 reload");

    if (vp.w === 1280) {
      const visualMaterials = await page.evaluate(() => {
        const T = window.__test;
        T.setPerfMode("high");
        T.setup(["dragon"], ["titan"]);
        const legend = document.querySelector("#playerField .card.rarity-legendary");
        legend.classList.add("foil", "can-attack");
        const frame = legend.querySelector(".frame-sheen");
        const frameStyle = getComputedStyle(frame);
        const foilStyle = getComputedStyle(legend, "::after");
        const hp = document.querySelector(".hp-badge");
        const mana = document.querySelector(".mana-badge");
        const hpStyle = getComputedStyle(hp);
        const manaStyle = getComputedStyle(mana);
        const frameAnimation = frameStyle.animationName;
        const frameMask = frameStyle.maskImage !== "none" ? frameStyle.maskImage : frameStyle.webkitMaskImage;
        ...
        const tonePoses = ["opponentHalden", "opponentVey", "opponentScarra"].map((name) => {
          const result = window.__capture.pose(name);
          return { name:result.name, opponent:document.body.dataset.opponent, hero:document.querySelector("#enemyHero .name")?.textContent || "" };
        });
        ...
        return {
          ...
          foilLayers,
          foilOpacity,
          hpGem: hpStyle.backgroundImage.includes("radial-gradient") && hpStyle.boxShadow.includes("inset"),
      ...
      assert(visualMaterials.frameAnimation.includes("legendFrameSweep"),
        `r61 visual: battle 傳說框掃光動畫生效（${visualMaterials.frameAnimation || "none"}）`);
      assert(visualMaterials.foilLayers >= 2,
        `r61 visual: battle 閃卡使用雙層虹彩色散（${visualMaterials.foilLayers} layers）`);
      assert(visualMaterials.foilOpacity > .5,
        `r61 visual: battle 閃卡色散保持可見（opacity ${visualMaterials.foilOpacity}）`);
      assert(visualMaterials.hpGem && visualMaterials.manaGem && visualMaterials.critical && visualMaterials.statusRing,
        "r61 visual: 英雄血量/法力採立體寶石語言，低血與可攻擊狀態不覆蓋身份材質");
      assert(visualMaterials.avatarFrame, "r61 visual: 英雄頭像有徑向底、雙圈描邊與 inset 工藝");
      assert(visualMaterials.captureOk && visualMaterials.captureFrozen
        && ["legendTauntFoil", "heroCritical", "fourRarityHand", "enemyTripleField", "opponentHalden", "opponentVey", "opponentScarra"].every((name) => visualMaterials.poseNames.includes(name))
        && visualMaterials.tripleField,
        "r61 capture: 命名 battle pose 一鍵佈景、敵場正名並鎖定傳說/foil 靜幀");
      ...
      await page.waitForFunction(() => [...document.querySelectorAll("#playerField .card img, #enemyField .card img")]
        .filter((img) => /hero(?:SerHalden|MagisterVey|Scarra|IsoldLongdusk|RuneFrostfang|MoenTidearbiter)\.png$/.test(img.getAttribute("src") || ""))
        .every((img) => img.complete && img.naturalWidth > 0));
      const battleHeroArt = await page.evaluate(() => {
        const cards = [...document.querySelectorAll("#playerField .card, #enemyField .card")];
        return {
          ids: cards.map((card) => card.dataset.cardId),
          sources: cards.map((card) => card.querySelector(".art img")?.getAttribute("src") || ""),
          fallbacks: cards.filter((card) => card.querySelector(".art-fallback")).length,
          loaded: cards.filter((card) => (card.querySelector(".art img")?.naturalWidth || 0) > 0).length,
        };
      });
      assert(battleHeroArt.ids.length === 6 && battleHeroArt.loaded === 6 && battleHeroArt.fallbacks === 0
        && battleHeroArt.sources.every((src) => /assets\/cards\/hero.+\.png$/.test(src)),
        "r61 hero art: 戰場六張角色卡皆載入專屬 PNG，無 emoji fallback");
      await page.evaluate(() => {
        const T = window.__test;
        T.setPerfMode("high");
      ...
      await page.evaluate(() => {
        const T = window.__test;
        T.setup(["golem"], ["guardian"]);
        const g = T.game();
        g.aiSmart = 2;
        g.enemyArchetype = "control";
        g.enemy.hand = [];
        g.enemy.deck = [];
        T.endTurn();
      });
      let aiGhostPeak = 0;
      const aiGhostDeadline = Date.now() + 3500;
      while (Date.now() < aiGhostDeadline) {
        const state = await page.evaluate(() => ({ turn:window.__test.game().turn, ghosts:window.__test.effects().ghosts }));
        aiGhostPeak = Math.max(aiGhostPeak, state.ghosts);
        if (state.turn === "player" && aiGhostPeak > 0) break;
        await sleep(20);
      }
      assert(aiGhostPeak === 2, `r61 AI FX guard: 單次隨從互毆只建立攻／受各一個 ghost（peak=${aiGhostPeak}）`);
      ...
      const reducedMaterials = await page.evaluate(() => {
        const legend = document.querySelector(".card.rarity-legendary");
        legend.classList.add("foil");
        return {
          frame: getComputedStyle(legend.querySelector(".frame-sheen")).animationName,
          foil: getComputedStyle(legend, "::after").animationName,
        };
      });
      assert(reducedMaterials.frame === "none" && reducedMaterials.foil === "none",
        "reduced-motion guard: 傳說框掃光與閃卡色散動畫停用");
    ...
    if (vp.w === 390) {
      const mobileCapture = await page.evaluate(() => {
        ...
        const data = {
          ...
          expanded:document.getElementById("handDrawerToggle").getAttribute("aria-expanded"),
          visible:styles.visibility !== "hidden" && hand.getBoundingClientRect().height > 0,
          cards:hand.querySelectorAll(".card").length,
        };
        window.__capture.clear();
        return data;
      });
      assert(mobileCapture.ok && mobileCapture.open && mobileCapture.expanded === "true" && mobileCapture.visible && mobileCapture.cards === 4,
        "r61 mobile capture: fourRarityHand 會展開抽屜並顯示四張卡");
    }

    if (vp.w === 1366) {
      const battleLowHit = {
        buttons: await areLocatorsHittable(page, ["#hintBtn", "#newGameQuickBtn", "#toPackQuickBtn", "#guideReplayBtn", "#endTurnBtn", "#missionDrawerBtn", "#kwCodexBtn", "#chronicleBtn"]),
        ...(await page.evaluate(() => {
    ...
    const difficultyHeroDecks = await page.evaluate(() => {
      const snapshots = {};
      localStorage.setItem("cardgame_opponent", "op_magister_vey");
      for (const difficulty of ["easy", "normal", "hard"]) {
        localStorage.setItem("cardgame_difficulty", difficulty);
        window.__newGame();
        snapshots[difficulty] = window.__test.aiDeckInfo().ids;
      }
      localStorage.setItem("cardgame_difficulty", "normal");
      return snapshots;
    });
    assert(!difficultyHeroDecks.easy.includes("heroMagisterVey")
      && difficultyHeroDecks.easy.filter((id) => id === "mage").length === 2
      && difficultyHeroDecks.normal.includes("heroMagisterVey")
      && difficultyHeroDecks.hard.includes("heroMagisterVey"),
      "AI hero mirror: easy uses the original replacement; normal/hard carry Vey's hero card");
    ...
    const attuneUi = await page.evaluate(() => {
      const T = window.__test;
      T.setup(["footman"], []);
      const g = T.game();
      g.turn = "player";
      const target = g.player.field[0];
      const before = { attack: target.attack, health: target.health };
      const moenUid = T.giveCard("heroMoenTidearbiter");
      T.playFromHand(moenUid);
      const pendingBeforeEnd = !!g.pendingBattlecry;
      T.endTurn();
    ...
    if (vp.w === 1280) {
      ...
      await page.evaluate(() => {
        const ids = ["heroSerHalden", "heroMagisterVey", "heroScarra", "heroIsoldLongdusk", "heroRuneFrostfang", "heroMoenTidearbiter"];
        window.__deckTest.testPack(ids.slice(0, 5).map((id) => window.getCardById(id)));
        window.__deckTest.setCollection(Object.fromEntries(ids.map((id) => [id, 1])));
        window.__deckTest.setCollectionFilters({ search: "", axis: "all", faction: "all", keyword: "all", rarity: "legendary", ownership: "owned", sort: "cost" });
      });
      // R67 起 .art 內疊有 .faction-emblem 徽記 img（factions 路徑）；本守門只驗 hero 主圖，徽記由 R67 專屬閘門驗
      await page.waitForFunction(() => [...document.querySelectorAll("#revealRow .card .art img:not(.faction-emblem), #collectionGrid .slot[data-card-id^='hero'] img:not(.faction-emblem)")]
        .every((img) => img.complete && img.naturalWidth > 0));
      const packHeroArt = await page.evaluate(() => ({
        reveal: [...document.querySelectorAll("#revealRow .card .art img:not(.faction-emblem)")].map((img) => img.getAttribute("src") || ""),
        collection: [...document.querySelectorAll("#collectionGrid .slot[data-card-id^='hero'][data-owned='1'] img:not(.faction-emblem)")].map((img) => img.getAttribute("src") || ""),
      }));
      assert(packHeroArt.reveal.length === 5 && packHeroArt.collection.length === 6
        && [...packHeroArt.reveal, ...packHeroArt.collection].every((src) => /assets\/cards\/hero.+\.png$/.test(src)),
        "r61 hero art: 開包五張展示與收藏六張縮圖皆使用專屬 PNG");
    }
    ...
    const heroPityCheck = await page.evaluate(() => {
      const T = window.__deckTest;
      T.setCollection({ heroSerHalden: 1 });
      T.setPity(19);
      T.setHeroPity(34);
      const common = window.getCardById("wolf");
      const allCommon = Array.from({ length: 5 }, () => Object.assign({}, common, { foil: false, tide: false }));
      const result = T.applyPity(allCommon);
      return {
        result,
        progress: document.getElementById("heroPityProgress")?.textContent || "",
        heroIds: [...window.HERO_SET_IDS],
      };
    });
    const forcedHeroId = heroPityCheck.result.cards.find((id) => heroPityCheck.heroIds.includes(id));
    assert(heroPityCheck.result.forced === true && heroPityCheck.result.heroForced === true
      && heroPityCheck.result.after === 0 && heroPityCheck.result.heroAfter === 0
      && heroPityCheck.result.hasRarePlus && heroPityCheck.result.hasHero
      && forcedHeroId && forcedHeroId !== "heroSerHalden"
      && heroPityCheck.progress.includes("0/35"),
      "35-pack hero pity composes with rare pity, resets both counters, and prefers an unowned hero");
    await page.evaluate(({ collection }) => {
      window.__deckTest.setCollection(collection);
      window.__deckTest.setCollectionFilters({ search: "", axis: "all", faction: "all", keyword: "all", rarity: "all", ownership: "all", sort: "cost" });
      window.__deckTest.setPity(0);
      window.__deckTest.setHeroPity(0);
    }, { collection: collectionForDeck(LEGAL_DECK_IDS) });
    if (vp.w <= 400) {
      await page.locator("#missionDrawerBtn").click();
    ...
    assert(packR36.textState.attr === "large" && packR36.textState.select === "large"
      && packR36.large > packR36.small && packR36.pwaVersion.includes("card-battle-r71-v1"),
      "開包戰績區顯示版本並可調整文字大小");
    assert(packR36.missionOpen && packR36.missionAria === "false" && /mission/.test(packMissionFocus),
      "開包任務抽屜開啟後焦點進入抽屜控制");
    assert(packR36.swGuard.key === "card_sw_auto_reload_r71_v1" && packR36.swGuard.windowMs === 15000 && packR36.swGuard.late === false,
