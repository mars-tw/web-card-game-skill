...
(() => {
  ...
  function swUrl() {
    return new URL(`../../sw.js?v=${window.__CARD_CACHE_VERSION || "card-battle-r63-v1"}`, location.href).toString();
  }

  const SW_BOOT = window.__CARD_SW_BOOT || {};
  const SW_AUTO_RELOAD_WINDOW_MS = SW_BOOT.SW_AUTO_RELOAD_WINDOW_MS || 15000;
  const SW_AUTO_RELOAD_KEY = SW_BOOT.SW_AUTO_RELOAD_KEY || "card_sw_auto_reload_r63_v1";
