---
name: web-card-game
description: 快速製作純原生（零依賴）網頁卡牌遊戲。當使用者想做卡牌對戰、抽卡/開卡包、牌組編輯器、集換式卡牌、TCG、爐石風格的網頁小遊戲時觸發。提供可直接執行的單頁 HTML/CSS/JS 模板，含關鍵字技能戰鬥、星級閃卡、正式牌組、主題切換，並用統一設定檔串接 Grok CLI 或 GPT API 生成卡牌美術。
...
- **牌組編輯器**：用收藏組成 20 張正式牌組，合法牌組可直接帶入對戰
- 兩者結合（完整 TCG 體驗，含主題切換）

## 主要功能

- **關鍵字技能系統**（益智、需思考）：嘲諷、衝鋒、突襲、吸血、戰吼、亡語、聖盾
- **星級變體**：每張卡有普通版與閃卡(foil)金色版，收集難度加倍
- **正式牌組**：固定 20 張；同卡最多 2 張、傳說最多 1 張；普通與閃卡擁有數合計
- **每日任務**：每日輪替 3 個任務，完成後領取 20-40 金幣
...
│   │   ├── cards.js          #   ★ 卡牌資料層（兩模板共用，單一事實來源，40 張）
│   │   ├── core.js           #   純規則層（出牌、攻擊、回合、清場亡語、stats/牌組/任務遷移、牌組驗證）
│   │   └── battle.js         #   對戰 UI/動畫/AI 殼層，規則委派給 core.js
│   └── card-pack/
│       ├── index.html        #   開卡包頁面 + 收藏冊 + 牌組編輯器
│       └── pack.js           #   抽卡機率、重複機制、開包動畫、牌組編輯器
...
3. **純規則層**：`core.js` 集中處理出牌合法性、攻擊/指定目標、回合推進、清場亡語、stats/牌組/每日任務遷移、牌組驗證與對戰牌庫洗牌；函式採 `(state, action, rng)` 風格，不碰 DOM、時間或全域亂數。
...
1. **收藏 → 牌組接通**：卡包頁用 `validateDeck(deckCardIds, collection, cardPool)` 驗證 `card_deck_v1`；
   合法牌組固定 20 張、同卡最多 2 張、傳說最多 1 張，普通與閃卡擁有數合計。
   `buildDeck(true)` 必須優先讀合法 `card_deck_v1` 並用 `buildBattleDeck(deckCardIds, cardPool, rng)` 洗牌；
   新玩家、壞存檔或不合法牌組才 fallback 到收藏/隨機補牌。AI 用隨機卡池（`buildDeck(false)`）。
   **驗收：卡包頁儲存的合法牌組，下一局對戰必須完整進玩家牌庫。**
2. **戰績 + 金幣經濟**：勝負存 `localStorage`（wins/losses/streak/coins），勝負畫面顯示連勝戰績；
   勝場簡單/普通/困難發 50/65/85 金幣，敗場發 15/20/30 金幣，金幣回開卡包消耗（首包免費）。
3. **治療上限用 maxHp**：`healHero` 用 `side.maxHp` 而非硬編碼，否則高血量難度會治療失效。
4. **重複卡有出口**：開到重複卡可分解成金幣（普 2/稀 8/史 25/傳 80），全重複一包期望回收約 37.1 金幣，低於包價 100 的一半。
5. **每日任務回流**：`card_quests_v1` 依 `dateSeed` 每日輪 3 個任務；對戰與開包事件都要餵進 `applyQuestProgress()`，領取透過 `claimQuest()` 加金幣。
